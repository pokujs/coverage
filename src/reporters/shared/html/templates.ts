...
const buildBreadcrumb = (
  ...
  if (backBreadcrumb)
    return `<a href="javascript:history.back()">Back</a> ${html.escape(currentLabel)}`;

  if (breadcrumb.length === 0) return 'All files';

  const links = breadcrumb.map(
    (breadcrumbEntry) =>
      `<a href="${html.escape(relativeHref(pagePath, breadcrumbEntry.pagePath))}">${html.escape(breadcrumbEntry.label)}</a>`
  );

  return `${links.join(' / ')} ${html.escape(currentLabel)}`;
};

export const renderHeader = (input: HtmlPageTemplateInput): string => {
  const assetLink = (filename: string): string =>
    html.escape(relativeHref(input.pagePath, filename));

  return `<!doctype html>
<html lang="en">
<head>
    <title>Code coverage report for ${html.escape(input.title)}</title>
    <meta charset="utf-8" />
