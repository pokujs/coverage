...
it('MODE resolves to a non-empty string via top-level ||', () => {
  assert.ok(typeof MODE === 'string' && MODE.length > 0);
});

it('firstNonEmpty exercises every arm of the || chain', () => {
...
it('firstDefined exercises every arm of the ?? chain', () => {
...
it('allTruthy short-circuits on the first falsy argument', () => {
